...
CREATE TABLE public.inspections (
  ...
  date date NOT NULL,
  notes text,
  payload jsonb NOT NULL, -- rooms/items/conditions
  created_at timestamptz DEFAULT now(),
  updated_at timestamptz DEFAULT now()
);
...
-- View to get inspection with property details
CREATE VIEW public.inspections_with_property AS
SELECT 
  i.*,
...
SELECT 
  ...
  COUNT(DISTINCT ex.id) as export_count
FROM public.users u
LEFT JOIN public.entitlements e ON u.id = e.user_id
LEFT JOIN public.properties p ON u.id = p.user_id
LEFT JOIN public.inspections i ON u.id = i.user_id
LEFT JOIN public.photos ph ON i.id = ph.inspection_id
LEFT JOIN public.exports ex ON i.id = ex.inspection_id
GROUP BY u.id, u.email, e.paid, e.source;
